...
public class UserController : BaseController, ICarterModule
{
	...
	{
		return await GenericController(async () => await userService.SetAdmin(username, toAdmin), logService,
			httpContext);
	}
	...
	{
		return await GenericController(() => Task.FromResult(userService.GetAllFromAD()), logService, httpContext);
	}
	...
	private static async Task<JsonHttpResult<ApiResponse>> GetAll(IUserService userService, ILogService logService,
		HttpContext httpContext)
	{
		return await GenericController(async () => await userService.GetAll(), logService, httpContext);
	}
	...
	private static async Task<JsonHttpResult<ApiResponse>> GetByUsername([Required] string username,
		IUserService userService, ILogService logService, HttpContext httpContext)
	{
		return await GenericController(async () => await userService.GetByUsername(username), logService, httpContext);
	}
	...
	{
		return await GenericController(async () => await userService.GetById(id), logService, httpContext);
	}
	...
	{
		return await GenericController(async () => await userService.Update(username, dtoUser), logService,
			httpContext);
	}
