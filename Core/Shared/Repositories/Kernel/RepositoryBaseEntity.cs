...
namespace Core.Shared.Repositories.Kernel
{
    ...
    /// <typeparam name="T"> Type that defines an table in the database and have to implement <see cref="IBaseEntity{T}"/></typeparam>
        ...
        where TContext : DbContext
    {
            ...
            if (includes != null)
            {
                foreach (var include in includes)
                {
                    if (include.Key != "")
					{
						...
						query = include.Value.Aggregate(query, (current, value) => current.Include(include.Key + "." + value));
					}
					else
		...
		{
			return await Query(filters, orderBy, withTracking, maxCount, includes: includes).ToListAsync();
		}
        ...
        {
            return await Query(
				filters: new Expression<Func<T, bool>>[] { predicate },
				null,
				withTracking,
				null,
