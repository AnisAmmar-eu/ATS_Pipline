...
public static class KPIInitializer
{
	...
	private static void InitializeStationForServer(AnodeCTX anodeCTX, int stationID)
	{
		...
		anodeCTX.KPIC.Add(new KPIC(
			KPICRID.StationCycleNotSigned + stationID,
		...
		anodeCTX.KPIC.Add(new KPIC(
			KPICRID.StationCycleMatchingCam1 + stationID,
		...
		// == Anode count by stations and type ==
		anodeCTX.KPIC.Add(new KPIC(
			KPICRID.AnodesStation + stationID,
			"Number of anodes for station" + stationID,
