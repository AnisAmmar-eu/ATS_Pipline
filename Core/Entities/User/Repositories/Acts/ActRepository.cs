...
namespace Core.Entities.User.Repositories.Acts
{
	public class ActRepository : RepositoryBaseEntity<AlarmCTX, Act, DTOAct>, IActRepository
	{
		...
		private static Expression<Func<Act, bool>>[] QueryFilters(string rid, string? entityType = null, string? parentType = null)
		{
			IEnumerable<Expression<Func<Act, bool>>> filters = new Expression<Func<Act, bool>>[] { a =>
				a.RID == rid
		...
		public async Task<Act> GetByRIDAndTypeWithIncludes(string? rid, string? entityType, string? parentType, bool withTracking = true)
		{
			if (rid == null)
