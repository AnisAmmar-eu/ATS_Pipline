...
namespace Core.Entities.User.Services.Acts
{
    public class ActsService : IActsService
    {
        ...
        public async Task<List<DTOActEntityStatus>> ActionsFromList(HttpContext httpContext, List<DTOActEntityStatus> dtoActEntitiesStatus)
        {
        ...
        {
            Act act = await _unitOfWork.Acts.GetByRIDAndTypeWithIncludes(dtoActEntityStatus.Act?.RID, dtoActEntityStatus.Act?.EntityType, dtoActEntityStatus.Act?.ParentType);
            ActEntity actEntity;

			try
            {
                actEntity = await _unitOfWork.ActEntities.GetByActWithIncludes(act, dtoActEntityStatus.EntityID, dtoActEntityStatus.ParentID);
            }
            catch (Exception e)
            {
                ...
                switch (actEntity.SignatureType)
                {
                    ...
                    case SignatureTypeRID.SESSION:
                        if (actEntity.ActEntityRoles.Count == 0)
                            ...
                            dtoActEntityStatus.Visible = actEntity.ActEntityRoles.Any(aer => aer.ApplicationType == ApplicationTypeRID.ROLE ?
        ...
        {
            Act act = await _unitOfWork.Acts.GetByRIDAndTypeWithIncludes(dtoActEntity.Act?.RID, dtoActEntity.Act?.EntityType, dtoActEntity.Act?.ParentType);
            ActEntity actEntity;

            try
            {
			    actEntity = await _unitOfWork.ActEntities.GetByActWithIncludes(act, dtoActEntity.EntityID, dtoActEntity.ParentID);
            }
            catch (Exception e)
            {
                if (e is not EntityNotFoundException)
        ...
        public async Task AssignAction(DTOActEntity dtoActEntity, bool remove = true)
        {
            ...
            else
            {
                ...
                if (actEntity.ActEntityRoles != null && remove == true)
			...
			switch (signatureType)
            {
                ...
                case SignatureTypeRID.EXPLICIT:
                    ...
                    {
                        foreach (DTOActEntityRole application in dtoActEntity.Applications.FindAll(a => a.Type == ApplicationTypeRID.ROLE))
                        {
                            ...
                            actEntity.ActEntityRoles?.Add(new(actEntity, role));
                        }
					}
                    if (dtoActEntity.Applications.Any(a => a.Type == ApplicationTypeRID.USER))
                    {
                        foreach (DTOActEntityRole application in dtoActEntity.Applications.FindAll(a => a.Type == ApplicationTypeRID.USER))
                        {
							...
							actEntity.ActEntityRoles?.Add(new(actEntity, user));
                        }
					}
                    break;
                default:
        ...
        /// <returns></returns>
        public async Task DeleteActionEntity(string actRID, string? entityType = null, int? entityID = null, string? parentType = null, int? parentID = null)
        {
        ...
        {
			Act actFromDuplicate = await _unitOfWork.Acts.GetByRIDAndTypeWithIncludes(dtoActToDuplicate.Act?.RID, dtoActToDuplicate.Act?.EntityType, dtoActToDuplicate.Act?.ParentType);
			Act act = await _unitOfWork.Acts.GetByRIDAndTypeWithIncludes(dtoAct.Act?.RID, dtoAct.Act?.EntityType, dtoAct.Act?.ParentType);

			ActEntity? actEntityToDuplicate;

			try
            {
                actEntityToDuplicate = await _unitOfWork.ActEntities.GetByActWithIncludes(actFromDuplicate, dtoActToDuplicate.EntityID, dtoActToDuplicate.ParentID);
            }
			catch (Exception e)
			{
				if (e is not EntityNotFoundException)
            ...
            {
                actEntity.ActEntityRoles.Add(new()
                {
                    ActEntity = actEntity,
        ...
        public async Task<string> HasRights(HttpContext httpContext, DTOActEntityToValid dtoActEntityToValid)
        {
            ...
            Act act = await _unitOfWork.Acts.GetByRIDAndTypeWithIncludes(dtoActEntityToValid.Act?.RID, dtoActEntityToValid.Act?.EntityType, dtoActEntityToValid.Act?.ParentType);

            ActEntity actEntity;

            try
            {
                actEntity = await _unitOfWork.ActEntities.GetByActWithIncludes(act, dtoActEntityToValid.EntityID, dtoActEntityToValid.ParentID);
            }
            catch (Exception e)
            {
                if (e is not EntityNotFoundException)
            ...
            else
            {
                switch (actEntity.SignatureType)
                {
                    ...
                    case SignatureTypeRID.EXPLICIT:
                        ...
                        if (dtoActEntityToValid.Login != null && dtoActEntityToValid.Login.Username != null && dtoActEntityToValid.Login.Password != null)
                        {
                            var otherUser = await _authService.CheckCredentials(dtoActEntityToValid.Login);
                            if (otherUser != null)
                            {
                                var rolesId = await _authService.GetRolesIdFromUser(otherUser, httpContext);
                                ...
                                {
                                    authorized = actEntity.ActEntityRoles.Any(aer => (rolesId.Contains(aer.ApplicationID) && aer.ApplicationType == ApplicationTypeRID.ROLE)
                                        || (aer.ApplicationID == otherUser.Id && aer.ApplicationType == ApplicationTypeRID.USER));
                                }
                            }
                            else
                                authorized = false;
        ...
        public bool ValidActionToken(HttpContext httpContext, List<DTOActEntityToValid> dtoActEntitiesToValid)
        {
            ...
            if (httpContext.User.Claims.Where(x => x.Type == "Id").Select(c => c.Value).FirstOrDefault() != actionTokenData.Claims.Where(x => x.Type == "UserId").Select(c => c.Value).FirstOrDefault())
                return false;

            int count = 0;
            foreach (DTOActEntityToValid dtoActEntityToValid in dtoActEntitiesToValid)
            {
                if (actionTokenData.Claims.Where(x => x.Type == "ActionRID").Select(c => c.Value).FirstOrDefault() == dtoActEntityToValid.Act?.RID
                    && (dtoActEntityToValid.EntityID == null || actionTokenData.Claims.Where(x => x.Type == "EntityId").Select(c => c.Value).FirstOrDefault() == dtoActEntityToValid.EntityID.ToString())
                    && (dtoActEntityToValid.ParentID == null || actionTokenData.Claims.Where(x => x.Type == "ParentId").Select(c => c.Value).FirstOrDefault() == dtoActEntityToValid.ParentID.ToString())
                    && (dtoActEntityToValid.Act?.EntityType == null || actionTokenData.Claims.Where(x => x.Type == "EntityType").Select(c => c.Value).FirstOrDefault() == dtoActEntityToValid.Act?.EntityType)
                    && (dtoActEntityToValid.Act?.ParentType == null || actionTokenData.Claims.Where(x => x.Type == "ParentType").Select(c => c.Value).FirstOrDefault() == dtoActEntityToValid.Act?.ParentType)
                    && actionTokenData.Claims.Where(x => x.Type == "Status").Select(c => c.Value).FirstOrDefault() == "Complete")
                    break;
